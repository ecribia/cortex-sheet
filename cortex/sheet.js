...
function add_event_handlers(editable)
{
	editable.addEventListener("blur",function (event)
	{
		...
		setTimeout(() => {
			...
			if (parentAttribute) {
				attachDiceIconToAttribute(parentAttribute, event.target);
			} else if (event.target.tagName === 'H2' || event.target.classList.contains('inline')) {
				const cTag = event.target.querySelector('c');
				if (cTag) {
					attachDiceIcon(event.target);
				}
			}
			scanExistingHeaders();
		}, 50);
...
function update_attribute_positions()
{
	...
	{
		var bottomCount = attributes.length;
		var topCount = 0;
		
		if (attributes.length >= 5) {
			bottomCount = Math.ceil(attributes.length / 2);
			topCount = attributes.length - bottomCount;
		}
		...
		for (var i = 0; i < attributes.length; i++)
		{
			...
			if (i < bottomCount) {
				...
				var y = topTop - Math.sin(alpha * 3.1415926535) * height;
				a.style.top = y + "mm";
    ...
    attributes.forEach(attr => {
        ...
        newCTag.addEventListener('click', (e) => {
            if (!editModeEnabled) {
                ...
                const dieValue = getDieFromAttribute(attr);
                if (!dieValue) return;
                
                const nameDiv = attr.querySelector('div[contenteditable]');
                const nameText = nameDiv ? nameDiv.textContent.trim() : 'Attribute';
                
                dicePool.push({ 
                    header: nameDiv,
                    attributeElement: attr,
                    value: dieValue, 
                    selected: false,
                    isAttribute: true
                });
